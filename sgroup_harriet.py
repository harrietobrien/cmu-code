...
class Sgroup(object):
    ...
    Corresponding to each Hall number key is a set of nested dictionaries
    containing information on the representatives of the Wyckoff position and
    its suborbits.
    
    General-position orbits for each Wyckoff position subgroup are decomposed 
    into suborbits (coordinates), all of the same multiplicity.
    
    Wyckoff position subgroups are denoted by their 'Multiplicity' and 
    'Wyckoff Letter', both of which are represented as keys that access an
    ordered list of multiplicites and Wyckoff letters for all subgroup 
    Wyckoff positions.
    
    Coordinates (suborbits) for each Wyckoff position multiplicity (subgroup)
    are grouped accordingly in lists (i.e. each element of the multiplicity 
    list, accessed by the keyword 'Multiplicity', corresponds to a sublist
    within the coordinate list, accessed by the keyword 'Coordinates').
    
    Coordinate lists accessed directly from spglib_database.py contain sublists
    of coordinates expressed in strings (for instance, "(-x,-y+1/2,z)") and are
    stored, for a given space group number, in self.coordinates.
    
    The string coordinates are parsed and used to construct diagonal matrix 
    representations of constraints on and translation vectors of the
    Wyckoff positions.
    
    Likewise, keys 'Multiplicity' and 'Site Symmetry', for a given hall number,
    store a list of string multiplicities and site symmetries in 
    self.multiplicity and self.site_symmetries, respectively. Site symmetries
    remain strings used to retrieve information in subsequent functions while
    the string elements of self.multiplicity are converted to integer values
    stored in self.wmult.
    '''
